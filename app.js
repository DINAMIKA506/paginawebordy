  ...
  app.innerHTML = `
    <main class="workspace">
      <aside class="sidebar">
        ...
        <nav class="side-nav" aria-label="Navegación del espacio">
          ${navButton("home", "⌂", "Inicio")}
          ${navButton("library", "▣", "Biblioteca")}
          ${navButton("tasks", "✓", "Tareas")}
          ${navButton("quick", "⚡", "Accesos")}
...
function renderNextTask(task) {
  ...
  return `<div class="next-item"><span class="next-date">${dayName}<br>${dayNumber}</span><div><b>${esc(task.title)}</b><span>${esc(task.project)} · Energía ${esc(task.energy.toLowerCase())}</span></div><button class="btn btn-icon" data-action="edit-task" data-id="${task.id}" aria-label="Editar ${esc(task.title)}">›</button></div>`;
}
...
function filterTasks() {
  return state.tasks.filter((task) => {
    ...
    const queryMatch = !query || [task.title, task.project, task.type, task.note].join(" ").toLowerCase().includes(query);
    ...
    return queryMatch && (ui.taskProject === "all" || task.project === ui.taskProject) && (ui.taskStatus === "all" || task.status === ui.taskStatus) && (ui.taskPriority === "all" || task.priority === ui.taskPriority) && (ui.taskEnergy === "all" || task.energy === ui.taskEnergy) && dateMatch;
...
function renderTasks() {
  ...
  return `
    <header class="view-head"><div><h2>Tareas</h2><p>Cliente → tipo de trabajo → próxima acción → fecha → energía.</p></div><div class="head-actions"><button class="btn btn-primary btn-small" data-action="open-task-form">＋ Nueva tarea</button></div></header>
    ...
    <div class="toolbar"><div class="toolbar-search"><input type="search" value="${esc(ui.taskQuery)}" data-action="task-search" placeholder="Buscar tarea, cliente, tipo o nota…" aria-label="Buscar tareas" /></div>${filterSelect("task-project", "Proyecto", ui.taskProject, ["all", ...projects])}${filterSelect("task-status", "Estado", ui.taskStatus, ["all", ...STATUS])}${filterSelect("task-priority", "Prioridad", ui.taskPriority, ["all", ...PRIORITIES])}${filterSelect("task-energy", "Energía", ui.taskEnergy, ["all", ...ENERGIES])}<div class="segmented"><button class="${ui.taskView === "list" ? "active" : ""}" data-action="task-view" data-value="list">Lista</button><button class="${ui.taskView === "project" ? "active" : ""}" data-action="task-view" data-value="project">Cliente</button><button class="${ui.taskView === "energy" ? "active" : ""}" data-action="task-view" data-value="energy">Energía</button></div></div>
...
function saveTask(form, data) {
  ...
  const item = { id: task?.id || uid("task"), project: data.project.trim(), type: data.type.trim(), title: data.title.trim(), dueDate: data.dueDate, status: data.status, priority: data.priority, energy: data.energy, url: safeUrl(data.url), note: data.note.trim(), createdAt: task?.createdAt || nowIso(), updatedAt: nowIso() };
