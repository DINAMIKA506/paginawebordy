...
const { buildOceanTemplate } = require("./templates");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (rows[0]?.data_json) return json(res, 200, { ok: true, ocean: rows[0].data_json, updatedAt: rows[0].updated_at || null });
      const timestamp = new Date().toISOString();
      const ocean = buildOceanTemplate({ displayName: current.profile.display_name });
      await supabaseRequest("ordy_user_oceans?on_conflict=user_id", {
        method: "POST",
        headers: { Prefer: "resolution=merge-duplicates,return=minimal" },
        body: JSON.stringify({ user_id: current.profile.id, data_json: ocean, updated_at: timestamp })
      });
      return json(res, 200, { ok: true, ocean, updatedAt: timestamp, created: true });
    }
