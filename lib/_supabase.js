...
function siteUrl(req) {
  ...
  if (requestOrigin && requestHost) {
    try {
      ...
      if (["http:", "https:"].includes(parsedOrigin.protocol) && parsedOrigin.host.toLowerCase() === requestHost) {
  ...
  if (configured) {
    try {
      ...
      if (["http:", "https:"].includes(parsedConfigured.protocol)) return parsedConfigured.origin;
    } catch (_) {}
  }
