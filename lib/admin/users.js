...
const { MODULES, buildOceanTemplate, normalizeTemplateKey, normalizeModules } = require("../templates");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const contacts = await supabaseRequest(`ordy_contacts?id=eq.${encodeURIComponent(contactId)}&select=id,email,delivered_at&limit=1`, { method: "GET" });
