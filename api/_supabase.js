...
function getConfig() {
  ...
  if (!url || !serviceKey || !anonKey) throw configurationError("Supabase todavía no está conectado en Vercel.");
...
async function requestJson(url, options = {}, fallback = "No fue posible completar la operación.") {
  ...
  if (!response.ok) {
    ...
    throw Object.assign(new Error(message), { statusCode: response.status, details: payload });
  }
  return payload;
}
